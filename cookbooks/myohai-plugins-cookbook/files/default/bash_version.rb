...
Ohai.plugin(:Bash) do
  ...
  def bash_vulnerable?
    cmd = <<ENDCMD
      env x='() { :;}; echo Your bash is very likely vulnerable as this exited 0. Non-vulnerable bash will exit 1' bash -c 'echo this is a test' | grep 'Your bash' > /dev/null 2>&1
ENDCMD
